...
async function main() {
    ...
    await prisma.veterinaryUser.deleteMany();
    await prisma.veterinary.deleteMany();


    await prisma.country.createMany({ data: countries })

    const verterinary = await prisma.veterinary.create({
        data: {
            name: "Hamp Veterinary",
            subscription: 'free',
            image: '',
            url: 'http://veterhamp.cl',
            created_by: 'prisma'
        }
    });

    const verterinary2 = await prisma.veterinary.create({
        data: {
            name: "Testing Veterinary",
            subscription: 'free',
            image: '',
            url: 'http://vetertesting.cl',
            created_by: 'prisma'
        }
    });
    ...
    const branch = await prisma.branch.create({
        data: {
            veterinaryId: verterinary.id,
            email: 'veter@hamp.cl',
            address: 'Manuel montt torres 812',
            phone: '933705876',
            city: 'San Antonio',
            countryId: 'CL',
            created_by: 'prisma'
    ...
    const branch2 = await prisma.branch.create({
        data: {
            veterinaryId: verterinary2.id,
            email: 'veter@testing.cl',
            address: 'Manuel montt torres 812',
            phone: '933705876',
            city: 'San Antonio',
            countryId: 'CL',
    ...
    const scheduledDays = await prisma.day.findMany();
    ...
    const veterinaryUser = await prisma.veterinaryUser.create({
        data: {
            veterinaryId: verterinary.id,
            userId: 'clw48yev4000012mbvxldnyyd',
            role: 'super',
            created_by: 'prisma'
        }
    })

    const veterinaryUser2 = await prisma.veterinaryUser.create({
        data: {
            veterinaryId: verterinary2.id,
            userId: 'clw48yev4000012mbvxldnyyd',
            role: 'super',
    ...
    await prisma.patient.createMany({
        data: [{
            ...
            race: null,
            gender: 'male',
            microship: Math.random().toString(),
            veterinaryId: verterinary.id,
            created_by: 'prisma'
        },
        {
